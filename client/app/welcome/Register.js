angular.module("welcome")
  .directive('register', function(ViewChannel) {
    function hide($el) {
      $el.animate({
        height: 0,
        margin:0
        ...
        '<form name="registrationForm" class="registration" novalidate="novalidate">' +
          '<div class="wrapper" style="height:0">' +
            '<div class="registration">' +
              '<input name="password" type="password" class="input-block-level" placeholder="Password longer then 5 characters"' +
            ...
            '<div class="control-group">' +
              '<input name="confirm" type="password" class="input-block-level" placeholder="Confirm Password"' +
                'ng-disabled="!validPassword()" ng-required="true" ng-change="passwordMatch(user.password, confirm)" ng-model="confirm"/>' +
            '</div>' +
            '<div class="control-group">' + 
              '<div class="span2">' +
                '<label class="radio inline">' +
                  'Admin' +
                  '<input class="span1" ng-model="user.role" name="role" type="radio" value="admin">' +
                '</label>' +
              '</div>' +
              '<label class="radio inline">' +
                'User' +
                '<input class="span1" ng-model="user.role" name="role" type="radio" value="user">' +
              '</label>' +
            '</div>' +
            '<div class="control-group">' +
            ...
            '</div>' +
            '<div class="control-group">' +
              '<input name="firstname" type="text" class="input-block-level" placeholder="Last Name"' +
                'ng-required="true" ng-model="user.lastname"/>' +
            '</div>' +
            '<div class="control-group">' +
              '<input name="email" type="email" class="input-block-level" placeholder="Email"' +
